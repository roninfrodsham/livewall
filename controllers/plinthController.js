...
exports.getLandingJsonData = (req, res) => {
	...
	let promises = [];
	promises.push(_getLandingJsonDataContentLeft(id, json.data.components));
	promises.push(_getLandingJsonDataTitleLeft(id, json.data.components));
		...
		PlinthData.find({'index' : idx}).limit(9).then((data) => {
			if(data.length > 0) {
				for (var i=0; i<data.length; i++)
				{
					...
					item.btnScreenID = `${uri}/api/plinth/${idx}/${data[i]._id}`;
					item.imageUrl = `${uri}/uploads/plinth/${idx}/${data[i].photo}`;
			...
			if(data.length > 0) {
				...
				for (var i=0; i<data.length; i++)
				{
					...
					if (data[i]._id == id)
					{
						...
						item.btnScreenID = `${uri}/api/plinth/${_resolveIndex(idx)}`;
						item.data = {};
						...
						if (data[i].sPhoto1 != "")
						{
							...
							item.data = {};
							item.data.imageUrl = `${uri}/uploads/plinth/${idx}/${data[i].sPhoto1}`;
						...
						if (data[i].sPhoto2 != "")
						{
							...
							item.data = {};
							item.data.imageUrl = `${uri}/uploads/plinth/${idx}/${data[i].sPhoto2}`;
						...
						if (data[i].sPhoto3 != "")
						{
							...
							item.data = {};
							item.data.imageUrl = `${uri}/uploads/plinth/${idx}/${data[i].sPhoto3}`;
						...
						if (data[i].sVideo != "")
						{
							...
							item.data = {};
							item.data.videoUrl = `${uri}/uploads/plinth/${idx}/${data[i].sVideo}`;
						...
						if (data[i].s360 != "")
						{
							...
							item.data = {};
							item.data.videoUrl = `${uri}/uploads/plinth/${idx}/${data[i].s360}`;
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
	const id = req.params.id;
    cb(null, `./public/uploads/plinth/${id}/`);
  },
...
exports.getData = (req, res) => {
	...
	if (_id) {
		findItemById(_id).then((result) => {
			...
			res.render('Plinth', { title: `Plinth ${id} Edit Artefact`, showAddForm, artefactEdit, artefactList, id, _id });
		});
  	
	} else {
		getItemList(id).then((result) => {
			artefactList = result;
			res.render('Plinth', { title: `Plinth ${id}`, showAddForm, artefactEdit, artefactList, id });
		}).catch((error) => {
			res.render('Plinth', { title: `Plinth ${id}`, showAddForm, artefactEdit, artefactList, id });
		});
	}
...
exports.getDataStandard = (req, res) => {
	...
	getItemList(id).then((result) => {
		artefactList = result;
		res.render('Plinth', { title: `Plinth ${id}`, showAddForm, artefactList, id });
	}).catch((error) => {
		res.render('Plinth', { title: `Plinth ${id}`, showAddForm, artefactList, id });
	});
};
...
exports.addItem = (req, res) => {
	...
	plinth.save().then((doc) => {
		res.redirect(`/plinth/${id}`);
	}).catch((e) => {
		res.status(400).send(e);
	});
};
...
exports.updateItem = (req, res) => {
	...
	{
		body.photo = photo;
		fs.unlink(`./public/uploads/plinth/${id}/${body.origPhoto}`, () => {
			console.log(`removed photo ./public/uploads/plinth/${id}/${body.origPhoto}`);
		});
	}
	if (sPhoto1 != "")
	{
		body.sPhoto1 = sPhoto1;
		fs.unlink(`./public/uploads/plinth/${id}/${body.origsPhoto1}`, () => {
	...
	if (sPhoto2 != "")
	{
		...
		fs.unlink(`./public/uploads/plinth/${id}/${body.origsPhoto2}`, () => {
	...
	if (sPhoto3 != "")
	{
		...
		fs.unlink(`./public/uploads/plinth/${id}/${body.origsPhoto3}`, () => {
	...
	{
		body.sVideo = sVideo;
		fs.unlink(`./public/uploads/plinth/${id}/${body.origsVideo}`, () => {
	...
	{
		body.s360 = s360;
		fs.unlink(`./public/uploads/plinth/${id}/${body.origs360}`, () => {
	...
	PlinthData.findByIdAndUpdate(_id, {$set: body}, {new: true}).then((entry) => {
		if (!entry) {
			return res.status(404).send();
		}
		res.redirect(`/plinth/${id}/${_id}`);
	}).catch((e) => {
		res.status(400).send(e);
	});
};
...
exports.deleteItem = (req, res) => {
	...
	PlinthData.findByIdAndRemove(_id).then((entry) => {
		if (!entry) {
			res.redirect(`/plinth/${id}`);
		}
		fs.unlink(`./public/uploads/plinth/${id}/${entry.photo}`, () => {
			console.log('removed photo');
		});
		fs.unlink(`./public/uploads/plinth/${id}/${entry.sPhoto1}`, () => {
			console.log('removed sPhoto1');
		});
		fs.unlink(`./public/uploads/plinth/${id}/${entry.sPhoto2}`, () => {
			console.log('removed sPhoto2');
		});
		fs.unlink(`./public/uploads/plinth/${id}/${entry.sPhoto3}`, () => {
			console.log('removed sPhoto3');
		});
		fs.unlink(`./public/uploads/plinth/${id}/${entry.sVideo}`, () => {
			console.log('removed sVideo');
		});
		fs.unlink(`./public/uploads/plinth/${id}/${entry.s360}`, () => {
			console.log('removed s360');
		});
		res.redirect(`/plinth/${id}`);
	}).catch((e) => {
...
const findItemById = (_id) => {
	return new Promise((resolve, reject) => {
		PlinthData.findOne({_id: _id}).then((entry) => {
			resolve({
				id: entry._id,
	    		title: entry.title,
	    		photo: entry.photo,
...
const getItemList = (id) => {
	return new Promise((resolve, reject) => {
		PlinthData.find({'index' : id}).sort( { timestamp: 1 } ).limit(9).then((entries) => {
